...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-semibold transition-transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]",
  {
    variants: {
      variant: {
        default:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, href, loading, children, ...props }, ref) => {
    ...
    if (asChild) {
      return (
        <Slot className={buttonClass} ref={ref}>
          {children}
        </Slot>
      )
    }
    ...
    if (href) {
      return React.createElement(
        'a',
        { className: buttonClass, href: href },
        loadingSpinner,
        children
      )
    }
