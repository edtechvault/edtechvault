  ...
  const registerTrigger = React.useCallback((node: HTMLButtonElement | null) => {
    setTriggerNodes((prev) => {
      if (node && !prev.includes(node)) {
        ...
        // When node is null (component unmounting), filter out disconnected nodes
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  React.useEffect(() => {
    ...
    if (currentBtn) {
      registerTrigger(currentBtn);
    }
  }, [registerTrigger]);

  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    const currentBtn = btnRef.current;
    const myIdx = triggerNodes.findIndex((n: HTMLButtonElement) => n === currentBtn);
  ...
  return (
    <button
      ...
      className={cn(
        'cursor-pointer focus-visible:ring-[var(--primary)]/30 inline-flex items-center gap-3 rounded-xl outline-none focus-visible:z-10 focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-60 transition-all duration-200',
        className,
      )}
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex items-center overflow-hidden justify-center size-8 shrink-0 border-2 border-[var(--border)] bg-[var(--background-white)] text-[var(--text-secondary)] rounded-full text-sm font-semibold transition-all duration-300',
        'data-[state=completed]:bg-[var(--success)] data-[state=completed]:text-white data-[state=completed]:border-[var(--success)]',
        'data-[state=active]:bg-[var(--primary)] data-[state=active]:text-white data-[state=active]:border-[var(--primary)] data-[state=active]:shadow-[var(--shadow-soft)]',
        className,
      )}
    >
...
function StepperSeparator({ className }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'm-0.5 rounded-full bg-[var(--border)] transition-all duration-300',
...
function StepperTitle({ children, className }: React.ComponentProps<'h3'>) {
  ...
  return (
    <h3
      ...
      data-state={state}
      className={cn(
        'text-sm md:text-base font-semibold leading-tight transition-colors duration-200',
        'data-[state=inactive]:text-[var(--text-secondary)]',
        'data-[state=active]:text-[var(--primary)]',
        'data-[state=completed]:text-[var(--text-primary)]',
        className,
      )}
    >
      {children}
...
function StepperDescription({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn('text-xs md:text-sm text-[var(--text-secondary)] leading-relaxed', className)}
    >
      {children}
    </div>
  );
